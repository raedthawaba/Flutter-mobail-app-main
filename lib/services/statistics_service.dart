...
class StatisticsService {
  ...
  // إحصائيات جغرافية حسب المناطق
  Future<Map<String, dynamic>> getGeographicStatistics() async {
    try {
      Map<String, dynamic> geoStats = {
        'regions': {},
        'topRegions': [],
        'distribution': {},
        'heatMap': {}
      ...
      // تحليل البيانات حسب المناطق
      Map<String, int> regionCounts = {};
      
      // إحصائيات الشهداء حسب المنطقة
      var martyrsSnapshot = await FirebaseFirestore.instance
          .collection(AppConstants.tableMartyrs)
          .get();
      
      for (var doc in martyrsSnapshot.docs) {
        String? location = doc.data()['location'] as String?;
        if (location != null && location.isNotEmpty) {
          regionCounts[location] = (regionCounts[location] ?? 0) + 1;
        }
      }
      
      // إحصائيات الجرحى حسب المنطقة
      var injuredSnapshot = await FirebaseFirestore.instance
          .collection(AppConstants.tableInjured)
          .get();
      
      for (var doc in injuredSnapshot.docs) {
        String? location = doc.data()['location'] as String?;
        if (location != null && location.isNotEmpty) {
          regionCounts[location] = (regionCounts[location] ?? 0) + 1;
        }
      ...
      geoStats['regions'] = regionCounts;
      
      // ترتيب المناطق حسب العدد
      List<MapEntry<String, int>> sortedRegions = regionCounts.entries.toList()
        ..sort((a, b) => b.value.compareTo(a.value));
      
      geoStats['topRegions'] = sortedRegions.take(10).map((entry) => {
        'region': entry.key,
        'count': entry.value,
        'percentage': (entry.value / regionCounts.values.reduce((a, b) => a + b) * 100).toStringAsFixed(1)
      }).toList();
      ...
      // توزيع النسب
      geoStats['distribution'] = _calculateDistribution(regionCounts);
      
      // بيانات الخريطة الحرارية
      geoStats['heatMap'] = _prepareHeatMapData(regionCounts);
      
      return geoStats;
    } catch (e) {
      print('خطأ في الإحصائيات الجغرافية: $e');
      return {
        'error': 'خطأ في تحميل الإحصائيات الجغرافية',
        'regions': {},
        'topRegions': [],
        'distribution': {},
        'heatMap': {}
  ...
  Future<Map<String, dynamic>> getDeepAnalytics() async {
    try {
      ...
      // تحليل الأنماط
      analytics['patterns'] = await _analyzePatterns();
      
      // استخراج الرؤى
      analytics['insights'] = await _extractInsights();
      
      // تحليل الاتجاهات
      analytics['trends'] = await _analyzeTrends();
      
      // التنبؤات
      analytics['predictions'] = await _generatePredictions();
      
      // الارتباطات
      analytics['correlations'] = await _analyzeCorrelations();
      
      return analytics;
    } catch (e) {
      print('خطأ في التحليل العميق: $e');
      return {
        'error': 'خطأ في التحليل العميق',
        'patterns': {},
        'insights': [],
        'trends': {},
        'predictions': {},
  ...
  Future<Map<String, dynamic>> getQuickStats() async {
    try {
      ...
      var martyrsSnapshot = await FirebaseFirestore.instance
          .collection(AppConstants.tableMartyrs)
          .get();
      quickStats['martyrsCount'] = martyrsSnapshot.docs.length;
      
      var injuredSnapshot = await FirebaseFirestore.instance
          .collection(AppConstants.tableInjured)
          .get();
      quickStats['injuredCount'] = injuredSnapshot.docs.length;
      
      var prisonersSnapshot = await FirebaseFirestore.instance
          .collection(AppConstants.tablePrisoners)
          .get();
      quickStats['prisonersCount'] = prisonersSnapshot.docs.length;
      
      quickStats['totalRecords'] = quickStats['martyrsCount'] + 
                                  quickStats['injuredCount'] + 
                                  quickStats['prisonersCount'];
      
      // حساب السجلات المضافة اليوم
      DateTime today = DateTime.now();
      DateTime startOfDay = DateTime(today.year, today.month, today.day);
      
      // هذا مجرد مثال، في التطبيق الحقيقي ستستخدم فلاتر التاريخ
      quickStats['todayAdded'] = 0; // Placeholder
      
      return quickStats;
    } catch (e) {
      print('خطأ في الإحصائيات السريعة: $e');
      return {
        'error': 'خطأ في تحميل الإحصائيات السريعة',
        'totalRecords': 0,
        'todayAdded': 0,
        'pendingReview': 0,
        'approved': 0,
